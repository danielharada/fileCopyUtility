...
def reviewAndCopy(copy_from_directory, copy_to_directory):
    review_window_in_hours = 24
    _review_window_in_sec = review_window_in_hours*3600

    os.chdir(copy_from_directory)
    text_files = getAllTxtFilesFromCurrentDirectory()
    files_with_age = createFileAgeDict(text_files)
    trimToOnlyModifiedInWindow(files_with_age, _review_window_in_sec)
...
def trimToOnlyModifiedInWindow(files_with_age, time_window):
    for each_file in files_with_age.keys():
...
if __name__ == '__main__':
    ...
    copy_from_directory = 'C:\\ReviewDaily'
    copy_to_directory = 'C:\\DailyTransfer'

    reviewAndCopy(copy_from_directory, copy_to_directory)
